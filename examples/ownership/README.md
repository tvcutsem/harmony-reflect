...
generic membranes (see `../generic_membranes.js`).
...
Here's the basic idea:
  ...
  1. Label your methods with "topics". A topic is a user-defined description
     that loosely categorizes methods according to various concerns.
     
     A method may be categorized according to multiple topics.
     
     Methods or functions can be labelled as follows:
     
         // label a function
         var sort = fn(["mutator","sorting"], function(array){...});
         
         // label a method in an object literal
         var iterator = {
           next: fn("iteration", function(){...}),
           hasNext: fn(["readonly","iteration"], function(){...})
         };
         ...
         Box.prototype.read = fn("readonly", function(){ return this.state; });
         Box.prototype.write = fn("mutator", function(v) { this.state = v; });

  2. Define "ownership boundaries" around strategically placed objects
     in the object graph.
     ...
     The `entry:` option identifies an object that is to be placed
     *inside* the boundary. The return value of the `Boundary` constructor
     is a proxy giving access to the box from *outside* the boundary.
     
     The `entry` object is named as such because this object typically forms
     the initial entry-point into the object graph encapsulated by the
     ...
     *inside* the boundary. Ideally, such a function should be closed (i.e.
     ...
     passed into the boundary. They can only store or compare the object
     reference.
  ...
  3. The boundary works its magic to enforce the restrictions imposed by the
     ...
     Then the following code throws:
         ...
         var boundedBoxMaker = new Boundary({
  ...
  4. The filters applied by boundaries are *transitive*. For example:
  
        var boxedCalculatorMaker = new Boundary({
          in: [],
          out: [".readonly"],
          entry: function() {
            var calculator = {
        ...
        var calc = boundBox.read(); // ok, read is marked "readonly"
        calc.square(4);
     ...
     boundary:
         ...
         shapes.forEach(function(shape) {
           ...
           // '[ "#forEach" ]' ('paint' is not classified)
         });
    
  5. References to incoming objects that are later passed outward again
  ...
  6. Boundaries can be nested, in which case filters are cumulative. That is,
     ...
     only methods marked ".foo" are accessible. For example:
...
correctly wrapped, and the `init` argument is protected by the in-filter (i.e.
...
HO contracts, like boundaries, must also transitively wrap values in order
